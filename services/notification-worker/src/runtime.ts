...
import {
  ...
  type NotificationDispatchApplication,
} from "./confirmation-dispatch";
...
export interface ServiceRuntime {
  ...
  readonly application: NotificationDispatchApplication;
  start(): Promise<void>;
  stop(): Promise<void>;
}
...
export function createRuntime(
  config: ServiceConfig,
  options?: { application?: NotificationDispatchApplication },
): ServiceRuntime {
    ...
    async start(): Promise<void> {
      ...
      serviceServer = http.createServer(async (request, response) => {
        ...
        try {
          ...
          const payload = await buildWorkloadResponse(route, context);
