...
function extractShowcasePayload(html) {
  const match = html.match(
    /<script id="showcase-data" type="application\/json">([\s\S]*?)<\/script>/,
  );
...
function validateDataArtifacts(payload) {
  ...
  ["OPT_REACT_TS_VITE_TANSTACK", "OPT_REACT_TS_NEXT_APP_ROUTER", "OPT_REACT_TS_REMIX"].forEach(
    (id) => {
  ...
  assert(
    chosenStack && chosenStack.option_id === "OPT_REACT_TS_VITE_TANSTACK",
    "Chosen frontend stack drifted.",
  );

  const upstreamAudienceSurfaceIds = new Set(
    parseCsvFile(REQUIRED_INPUTS.audienceSurfaceInventory).map((row) => row.surface_id),
  );
  const upstreamGatewaySurfaceIds = new Set(
    parseCsvFile(REQUIRED_INPUTS.gatewayMatrix).map((row) => row.gateway_surface_id),
  );
  ...
  assert(
    upstreamAudienceSurfaceIds.size === generatedAudienceSurfaceIds.size,
    "Audience surface coverage count drifted in gateway split matrix.",
  );
  assert(
    upstreamGatewaySurfaceIds.size === generatedGatewaySurfaceIds.size,
    "Gateway surface coverage count drifted in gateway split matrix.",
  );
  upstreamAudienceSurfaceIds.forEach((id) =>
    assert(generatedAudienceSurfaceIds.has(id), `Missing audience surface mapping: ${id}`),
  );
  upstreamGatewaySurfaceIds.forEach((id) =>
    assert(generatedGatewaySurfaceIds.has(id), `Missing gateway surface mapping: ${id}`),
  );
  ...
  assert(
    coverageRows.some((row) => row.viewport_matrix.includes("320")),
    "Playwright coverage must include 320px.",
  );
  assert(
    coverageRows.some((row) => row.viewport_matrix.includes("768")),
    "Playwright coverage must include 768px.",
  );
  assert(
    coverageRows.some((row) => row.viewport_matrix.includes("1024")),
    "Playwright coverage must include 1024px.",
  );
  assert(
    coverageRows.some((row) => row.viewport_matrix.includes("1440")),
    "Playwright coverage must include 1440px.",
  );

  assert(payload.summary.stack_option_count === stackRows.length, "Payload stack count drifted.");
  assert(
    payload.summary.gateway_surface_count === gatewayRows.length,
    "Payload gateway count drifted.",
  );
  assert(
    payload.summary.client_state_policy_count === stateRows.length,
    "Payload state policy count drifted.",
  );
  assert(
    payload.summary.ui_contract_publication_count === contractRows.length,
    "Payload UI contract count drifted.",
  );
  assert(
    payload.summary.playwright_coverage_count === coverageRows.length,
    "Payload Playwright coverage count drifted.",
  );
  assert(
    payload.summary.shell_demo_count === payload.shell_demos.length,
    "Payload shell demo count drifted.",
  );
  assert(payload.summary.unresolved_gap_count === 0, "Unexpected frontend gaps present.");
  assert(
    payload.chosen_stack_option_id === "OPT_REACT_TS_VITE_TANSTACK",
    "Payload chosen stack drifted.",
  );
  assert(
    payload.chosen_bff_pattern_option_id === "BFF_ROUTE_FAMILY_SPLIT",
    "Payload chosen BFF pattern drifted.",
  );
  assert(
    payload.chosen_contract_publication_posture ===
      "design_and_runtime_bundles_are_published_together",
    "Contract publication posture drifted.",
  );
}
...
function validateDocsAndAssets(payload) {
  ...
  const stateDoc = readText(
    path.join(DOCS_DIR, "14_client_state_data_fetch_and_live_update_baseline.md"),
  );
  const testingDoc = readText(
    path.join(
      DOCS_DIR,
      "14_frontend_testing_and_Playwright_or_other_appropriate_tooling_baseline.md",
    ),
  ...
  [stackDoc, gatewayDoc, shellDoc, designDoc, stateDoc, testingDoc, visualDoc].forEach(
    (text, index) => {
      assert(
        text.startsWith("# "),
        `Documentation file ${index + 1} is missing a top-level heading.`,
      );
    },
  );
  ...
  assert(
    designDoc.includes("DesignTokenExportArtifact"),
    "Design doc missing contract bundle coverage.",
  );
  assert(
    designDoc.includes("Finding 118") ||
      designDoc.includes("Finding 118".toLowerCase()) ||
      designDoc.includes("Finding 118 - Token export"),
    "Design doc should mention Finding 118 closure.",
  );
  ...
  assert(
    visualDoc.includes("Signal Atlas Live / Quiet Clarity"),
    "Visual doc missing named visual direction.",
  );
  ...
  assert(
    mermaid.includes("gws_governance_shell"),
    "Mermaid map missing governance gateway surface.",
  );
  ...
  assert(
    html.includes(payload.frontend_architecture_id),
    "Showcase payload is not embedded in HTML.",
  );
  ...
  [
    ...
    assert(
      ...
      `Showcase HTML contains forbidden remote asset reference: ${needle}`,
    );
  });
}
...
function validateShowcasePayload(payload) {
  assert(
    payload.frontend_architecture_id === "vecells_frontend_architecture_v1",
    "Unexpected frontend architecture id.",
  );
  assert(
    payload.mission && payload.mission.includes("Vecells frontend baseline"),
    "Mission field drifted.",
  );
  assert(
    Array.isArray(payload.source_precedence) && payload.source_precedence.length >= 10,
    "Source precedence looks incomplete.",
  );
  assert(
    payload.source_precedence[0].includes("Continuity key and shell law"),
    "Source precedence order drifted.",
  );
  const upstream = ensurePrerequisites();
  assert(
    JSON.stringify(payload.upstream_inputs) === JSON.stringify(upstream),
    "Upstream input summary drifted.",
  );
  assert(
    Array.isArray(payload.shell_demos) && payload.shell_demos.length === 4,
    "Shell demo count drifted.",
  );
  ...
  payload.shell_demos.forEach((shell) => {
    assert(
      Array.isArray(shell.routes) && shell.routes.length >= 2,
      `Shell demo needs at least two routes: ${shell.shell_id}`,
    );
    assert(
      Array.isArray(shell.anchor_options) && shell.anchor_options.length >= 3,
      `Shell demo anchor options drifted: ${shell.shell_id}`,
    );
    shell.routes.forEach((route) => {
      [
        ...
        assert(
          route[field],
          `Missing showcase route field ${field} on ${shell.shell_id}:${route.route_id}`,
        );
      });
    });
  });
  ...
  assert(
    chosenBff && chosenBff.option_id === "BFF_ROUTE_FAMILY_SPLIT",
    "Chosen BFF pattern drifted.",
  );
}
