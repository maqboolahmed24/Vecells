...
const GATES = JSON.parse(
  fs.readFileSync(
    path.join(ROOT, "data", "analysis", "nhs_login_live_gate_conditions.json"),
    "utf8",
  ),
);
...
async function run() {
  ...
  const targetUrl = process.env.NHS_LOGIN_PORTAL_URL ?? GATES.dry_run_defaults.default_target_url;
  ...
  if (!realMutationRequested) {
    const blockedGateCount = GATES.live_gate_conditions.filter(
