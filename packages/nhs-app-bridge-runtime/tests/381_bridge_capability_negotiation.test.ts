...
describe("381 bridge capability negotiation", () => {
  it("negotiates only runtime-verified capabilities and preserves script query hints", () => {
  ...
  it("blocks stale or mismatched matrices before bridge-backed actions execute", () => {
    const navigationContract = contract();
    const embeddedEligibility = eligibility();
    const matrix = negotiateBridgeCapabilityMatrix({
  ...
  it("clears back-action leases on route exit and fence drift", () => {
    ...
    const install = bridge.setBackAction(() => undefined);
    expect(install.ok).toBe(true);
    expect(bridge.snapshot().activeLeases).toHaveLength(1);

    const stale = bridge.clearForFenceDrift({ sessionEpochRef: "SessionEpoch:drifted" });
    expect(stale[0]?.leaseState).toBe("stale");
