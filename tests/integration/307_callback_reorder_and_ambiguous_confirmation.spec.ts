...
describe("307 callback reorder and ambiguous confirmation", () => {
  it("keeps callback confirmation pending until authoritative read confirms the booking", async () => {
    const flow = await setupBookingCoreFlow({
      seed: "307_read_after_write",
  ...
  it("collapses duplicate and stale callbacks onto the same pending transaction chain", async () => {
    const flow = await setupBookingCoreFlow({
      seed: "307_duplicate_callbacks",
    ...
    const duplicate = await flow.reconciliationApplication.assimilateBookingReceipt({
      ...
      commandSettlementRecordRef:
        `assimilate_receipt_settlement_${flow.seed}_duplicate`,
      ...
      schemaVerified: true,
      payloadArtifactRef:
        `artifact://booking/reconciliation/${flow.seed}/receipt/duplicate`,
    ...
    const stale = await flow.reconciliationApplication.assimilateBookingReceipt({
      ...
      commandSettlementRecordRef:
        `assimilate_receipt_settlement_${flow.seed}_stale`,
    ...
    expect(first.bookingCommit.transaction.authoritativeOutcomeState).toBe(
      "confirmation_pending",
    );
    expect(duplicate.bookingCommit.appointmentRecord).toBeNull();
    expect(stale.bookingCommit.appointmentRecord).toBeNull();
    expect(stale.bookingCommit.transaction.bookingTransactionId).toBe(
      created.transaction.bookingTransactionId,
    );
  });

  it("requires manual attention on secure-callback failure and resolves the dispute with one final booking", async () => {
    const flow = await setupBookingCoreFlow({
      seed: "307_manual_resolution",
      supplierRef: "vecells_local_gateway",
      integrationMode: "local_gateway_component",
      deploymentType: "practice_local_gateway",
      audience: "staff",
    ...
    const resolved = await flow.reconciliationApplication.resolveManualDispute({
      ...
      commandSettlementRecordRef:
        `resolve_manual_dispute_settlement_${flow.seed}`,
      ...
      payloadArtifactRef:
        `artifact://booking/reconciliation/${flow.seed}/manual_resolution`,
