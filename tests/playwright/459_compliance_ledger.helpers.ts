...
export const OUTPUT_DIR = path.join(ROOT, ".artifacts", "compliance-ledger-459");
...
export async function startOpsConsole(): Promise<StartedServer> {
  const child = spawn(
    "pnpm",
    ["exec", "vite", "--host", "127.0.0.1", "--port", "4320", "--strictPort"],
    {
      cwd: APP_DIR,
      env: process.env,
...
export async function withOpsCompliancePage(
