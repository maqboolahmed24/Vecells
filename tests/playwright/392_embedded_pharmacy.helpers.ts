...
export type EmbeddedPharmacyView = "choice" | "instructions" | "status" | "outcome" | "recovery";
...
async function allocatePort(start = 5392 + (process.pid % 500)): Promise<number> {
  for (let port = start; port < start + 220; port += 1) {
    if (await canUsePort(port)) return port;
  }
  throw new Error("No free localhost port found for 392 patient-web Playwright test.");
}
...
export async function startPatientWeb(): Promise<{ child: ChildProcess; baseUrl: string }> {
  ...
  try {
    await waitForHttp(`${baseUrl}/nhs-app/pharmacy/PHC-2048/choice?fixture=choice`);
    return { child, baseUrl };
  } catch (error) {
    child.kill("SIGTERM");
    throw new Error(`patient-web failed to start for 392.\n${logs.join("")}`, { cause: error });
  }
}
...
export function embeddedPharmacyUrl(
  baseUrl: string,
  options: {
    readonly pharmacyCaseId?: string;
    readonly view?: EmbeddedPharmacyView;
    readonly query?: string;
  } = {},
): string {
  const pharmacyCaseId = options.pharmacyCaseId ?? "PHC-2048";
  const view = options.view ?? "choice";
  const query = options.query ? `?${options.query.replace(/^\?/, "")}` : "";
  return `${baseUrl}/nhs-app/pharmacy/${pharmacyCaseId}/${view}${query}`;
}

export async function openEmbeddedPharmacy(page: any, url: string): Promise<void> {
  await page.goto(url, { waitUntil: "load" });
  await page.getByTestId("EmbeddedPharmacyFrame").waitFor();
  await assertRootContract(page);
}

export async function assertRootContract(page: any): Promise<void> {
  const root = page.getByTestId("EmbeddedPharmacyFrame");
  assertCondition(
    (await root.getAttribute("data-visual-mode")) === "NHSApp_Embedded_Pharmacy",
    "embedded pharmacy visual mode missing",
  );
  assertCondition(Boolean(await root.getAttribute("data-selected-provider")), "selected provider missing");
  assertCondition(
    (await root.getAttribute("data-continuity-state")) !== null,
    "continuity state missing",
  );
}

export async function clickPrimary(page: any): Promise<void> {
  await page.getByTestId("EmbeddedPharmacyActionReserve").locator("button").first().click();
}
