...
export const GOVERNANCE_APP_URL = "http://127.0.0.1:4327";
export const OPS_APP_URL = "http://127.0.0.1:4328";
export const OUTPUT_DIR = path.join(ROOT, ".artifacts", "backup-restore-channels-462");
...
export async function startBackupRestoreApps() {
  const governance = await startViteApp(
    GOVERNANCE_APP_DIR,
    "governance-console",
    4327,
    "/ops/config/backup-restore",
  );
  const ops = await startViteApp(OPS_APP_DIR, "ops-console", 4328, "/ops/resilience");
...
export function assertRedactedBackupPayload(payload: Record<string, unknown>) {
  const serialized = JSON.stringify(payload);
  assert(!serialized.match(/https?:\/\//), "backup payload must not include raw URLs");
  assert(!serialized.match(/accessToken|credential|rawObjectStoreUrl|inlineSecret|clinicalNarrative/));
  assert.equal(payload.schemaVersion, "462.phase9.fake-backup-target-payload.v1");
  assert.equal(typeof payload.safeDescriptorHash, "string");
  assert.equal(typeof payload.resilienceTupleHash, "string");
}

export function assertRedactedRestorePayload(payload: Record<string, unknown>) {
  const serialized = JSON.stringify(payload);
  assert(!serialized.match(/https?:\/\//), "restore report payload must not include raw URLs");
  assert(!serialized.match(/accessToken|credential|rawObjectStoreUrl|inlineSecret|clinicalNarrative/));
  assert.equal(payload.schemaVersion, "462.phase9.fake-restore-report-payload.v1");
  assert.equal(typeof payload.presentationGrantRef, "string");
  assert.equal(typeof payload.safeDescriptorHash, "string");
}

export async function withBackupRestoreBrowser(
  ...
  const apps = await startBackupRestoreApps();
