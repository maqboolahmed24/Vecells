...
async function allocatePort(start = 5089 + (process.pid % 500)): Promise<number> {
  for (let port = start; port < start + 200; port += 1) {
    if (await canUsePort(port)) return port;
  }
  throw new Error("No free localhost port found for 389 patient-web Playwright test.");
}
...
export async function startPatientWeb(): Promise<{ child: ChildProcess; baseUrl: string }> {
  ...
  try {
    await waitForHttp(`${baseUrl}/nhs-app/start-request?fixture=empty`);
    return { child, baseUrl };
  } catch (error) {
    child.kill("SIGTERM");
    throw new Error(`patient-web failed to start for 389.\n${logs.join("")}`, { cause: error });
  }
}
...
export function startRequestUrl(
  baseUrl: string,
  options: {
    readonly draft?: string;
    readonly step?: StartRequestStep;
    readonly fixture?: string;
  } = {},
): string {
  const draft = options.draft ?? "dft_389_playwright";
  const step = options.step ?? "request-type";
  const route =
    step === "request-type"
      ? `/nhs-app/start-request/${draft}/request-type`
      : `/nhs-app/start-request/${draft}/${step}`;
  const params = new URLSearchParams();
  if (options.fixture) params.set("fixture", options.fixture);
  return `${baseUrl}${route}${params.size > 0 ? `?${params.toString()}` : ""}`;
}

export async function openStartRequest(page: any, url: string): Promise<void> {
  await page.goto(url, { waitUntil: "load" });
  await page.getByTestId("EmbeddedIntakeFrame").waitFor();
  await assertRootContract(page);
}

export async function assertRootContract(page: any): Promise<void> {
  const root = page.getByTestId("EmbeddedIntakeFrame");
  assertCondition(
    (await root.getAttribute("data-visual-mode")) === "NHSApp_Embedded_Start_Request",
    "embedded start request visual mode missing",
  );
  assertCondition(
    (await root.getAttribute("data-route-family")) === "rf_intake_self_service",
    "embedded start request route family drifted",
  );
...
export async function clickPrimary(page: any): Promise<void> {
  await page.getByTestId("EmbeddedSubmitActionBar").locator("button").first().click();
}

export async function chooseOption(page: any, label: string): Promise<void> {
  const input = page.locator("label").filter({ hasText: label }).locator("input").first();
  await input.check();
}

export async function waitForSaveState(page: any, state: string): Promise<void> {
  await page.waitForFunction(
    (expected: string) =>
      document.querySelector("[data-testid='EmbeddedDraftSaveChip']")?.getAttribute("data-save-state") === expected,
    state,
  );
}
