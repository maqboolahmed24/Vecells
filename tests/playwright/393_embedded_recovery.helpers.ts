...
export type EmbeddedRecoveryView =
  | "expired-link"
  | "invalid-context"
  | "lost-session"
  | "unsupported-action"
...
async function allocatePort(start = 5393 + (process.pid % 500)): Promise<number> {
  for (let port = start; port < start + 220; port += 1) {
    if (await canUsePort(port)) return port;
  }
  throw new Error("No free localhost port found for 393 patient-web Playwright test.");
}
...
export async function startPatientWeb(): Promise<{ child: ChildProcess; baseUrl: string }> {
  ...
  try {
    await waitForHttp(`${baseUrl}/nhs-app/recovery/REC-393/expired-link?fixture=expired-link`);
    return { child, baseUrl };
  } catch (error) {
    child.kill("SIGTERM");
    throw new Error(`patient-web failed to start for 393.\n${logs.join("")}`, { cause: error });
  }
}
...
export function embeddedRecoveryUrl(
  baseUrl: string,
  options: {
    readonly journeyRef?: string;
    readonly view?: EmbeddedRecoveryView;
    readonly query?: string;
  } = {},
): string {
  const view = options.view ?? "expired-link";
  const journeyRef =
    options.journeyRef ??
    (view.startsWith("artifact") || view === "download-progress"
      ? "ART-393"
      : view === "route-freeze"
        ? "FRZ-393"
        : view === "degraded-mode"
  ...
  return `${baseUrl}/nhs-app/recovery/${journeyRef}/${view}${query}`;
}

export async function openEmbeddedRecovery(page: any, url: string): Promise<void> {
...
export async function assertRootContract(page: any): Promise<void> {
  const root = page.getByTestId("EmbeddedRecoveryArtifactFrame");
  assertCondition(
    (await root.getAttribute("data-visual-mode")) === "NHSApp_Embedded_Recovery_And_Artifacts",
    "embedded recovery visual mode missing",
  );
  assertCondition(Boolean(await root.getAttribute("data-route-key")), "route key missing");
  assertCondition(Boolean(await root.getAttribute("data-actionability")), "actionability missing");
}

export async function clickPrimary(page: any): Promise<void> {
  await page.getByTestId("EmbeddedRecoveryActionCluster").locator("button").first().click();
}
