...
export const OUTPUT_ROOT = path.join(ROOT, "output", "playwright", "479-dress-rehearsal");
...
export async function captureScreenshot(locator: any, relativePath: string): Promise<string> {
  ...
  assertNoSensitiveSerialized(fs.readFileSync(absolutePath), relativePath);
  return absolutePath;
}
...
export function writeSuiteArtifactManifest(
  ...
  const value = {
    recordType: "DressRehearsalSuiteArtifactManifest",
    suite,
...
export async function safeClickIfVisible(locator: any): Promise<boolean> {
...
export function readSeed(): any {
  return JSON.parse(
    fs.readFileSync(path.join(ROOT, "tests", "fixtures", "479_production_like_seed.json"), "utf8"),
  );
}
