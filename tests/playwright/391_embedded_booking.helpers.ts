...
export type EmbeddedBookingView = "offers" | "alternatives" | "waitlist" | "manage" | "confirmation" | "calendar" | "recovery";
...
async function allocatePort(start = 5391 + (process.pid % 500)): Promise<number> {
  for (let port = start; port < start + 220; port += 1) {
    if (await canUsePort(port)) return port;
  }
  throw new Error("No free localhost port found for 391 patient-web Playwright test.");
}
...
export async function startPatientWeb(): Promise<{ child: ChildProcess; baseUrl: string }> {
  ...
  try {
    await waitForHttp(`${baseUrl}/nhs-app/bookings/booking_case_391/offers?fixture=live`);
    return { child, baseUrl };
  } catch (error) {
    child.kill("SIGTERM");
    throw new Error(`patient-web failed to start for 391.\n${logs.join("")}`, { cause: error });
  }
}
...
export function embeddedBookingUrl(
  baseUrl: string,
  options: {
    readonly bookingCaseId?: string;
    readonly view?: EmbeddedBookingView;
    readonly query?: string;
  } = {},
): string {
  const bookingCaseId = options.bookingCaseId ?? "booking_case_391";
  const view = options.view ?? "offers";
  const query = options.query ? `?${options.query.replace(/^\?/, "")}` : "";
  return `${baseUrl}/nhs-app/bookings/${bookingCaseId}/${view}${query}`;
}

export async function openEmbeddedBooking(page: any, url: string): Promise<void> {
...
export async function assertRootContract(page: any): Promise<void> {
  const root = page.getByTestId("EmbeddedBookingFrame");
  assertCondition(
    (await root.getAttribute("data-visual-mode")) === "NHSApp_Embedded_Booking",
    "embedded booking visual mode missing",
  );
...
export async function clickPrimary(page: any): Promise<void> {
  await page.getByTestId("EmbeddedBookingActionReserve").locator("button").first().click();
}
