...
export const GOVERNANCE_APP_URL = "http://127.0.0.1:4333";
export const OPS_APP_URL = "http://127.0.0.1:4334";
export const OUTPUT_DIR = path.join(ROOT, ".artifacts", "live-projection-gateway-464");
...
export async function startLiveProjectionApps() {
  ...
  const ops = await startViteApp(OPS_APP_DIR, "ops-console", 4334, "/ops/overview");
...
export async function assertNoRawLivePayload(locator: any, label: string): Promise<void> {
  const serialized = String(await locator.evaluate((node: HTMLElement) => node.textContent ?? ""));
  assert(
    !serialized.match(/rawDomainEventRef|rawPayload|clinicalNarrative|patientNhs|nhsNumber/),
    `${label} exposed raw live event material`,
  );
  assert(
    !serialized.match(/Bearer|access_token|sk_live|BEGIN PRIVATE/),
    `${label} exposed secret material`,
  );
}

export async function withLiveProjectionBrowser(
  ...
  fs.mkdirSync(OUTPUT_DIR, { recursive: true });
  const apps = await startLiveProjectionApps();
