# Vecells Complete End-to-End System Flow (Audited Baseline)
...
%% ENTRY CHANNELS
%% =========================
subgraph ENTRY[All User Entry Channels]
  P_WEB[Patient uses web portal]
  P_PHONE[Patient calls phone or IVR]
  P_NHS[NHS App web jump-off path
  deferred channel]
end
...
%% IDENTITY + INTAKE
%% =========================
subgraph INTAKE[Unified Intake, Identity, Evidence, and Safety]
  I_AUTH[Authenticate or redeem AccessGrant]
  I_ID[Resolve identity, access scope, and claim
  anonymous, partial_match, matched, claimed]
  I_REPAIR[IdentityRepairCase
  wrong-patient correction, grant rotation,
  lineage-level identity hold until verified]
  I_ENVELOPE[Durable SubmissionEnvelope or call-session shell
  stable lineage, grants, and public IDs]
  I_CAPTURE[Capture details
  web form, live call, continuation flow]
  I_ATTACH[Quarantine uploads or audio
  scan, transcribe, classify evidence]
  I_QUAR[Artifact quarantine
  malware, unreadable, or unsupported evidence]
  I_READY{Evidence safety-usable
  urgent-live, or manual fallback?}
  I_CONTINUE[SMS continuation, challenge,
  ...
  I_URGENT_REQ[Urgent diversion required
  or urgent-live preemption]
  I_URGENT_DONE[Urgent advice or escalation
  durably issued]
  I_FAIL[FallbackReviewCase
  degraded receipt, manual safety review, SLA anchor]
  I_TASK[Create one TriageTask
  request enters triage_ready]
  I_RECEIPT[Issue single receipt
  ETA, status, and tracking]
end

%% Architectural correction: wrong-patient signals must enter a governed `IdentityRepairCase`
%% and attach lineage-level identity-hold metadata; they may not silently mutate patient binding in place.
P_WEB --> I_AUTH
P_NHS --> I_AUTH
I_AUTH --> I_ID
P_PHONE --> I_ID
I_ID -->|Wrong-patient suspicion or bind dispute| I_REPAIR
I_ID --> I_ENVELOPE --> I_CAPTURE
I_CAPTURE --> I_ATTACH --> I_READY
I_READY -->|Need more evidence| I_CONTINUE --> I_ATTACH
I_READY -->|Unsafe or unreadable evidence| I_QUAR --> I_FAIL
I_READY -->|Urgent-live| I_URGENT_REQ
I_READY -->|Ready to promote| I_SUBMIT --> I_DUP
I_DUP -->|Accepted retry| I_PRIOR
I_DUP -->|Existing lineage continuation| T_RESPONSE
I_DUP -->|New lineage| I_NORM --> I_SAFETY
I_DUP -.->|Review-required cluster| X_DUP
X_DUP -.-> T_QUEUE
I_SAFETY -->|Urgent| I_URGENT_REQ
I_URGENT_REQ --> I_URGENT_DONE
I_SAFETY -->|Screen clear or residual risk| I_TASK --> I_RECEIPT
I_SAFETY -->|Engine or ingest failure| I_FAIL --> I_RECEIPT

%% =========================
%% TRIAGE + HUMAN CHECKPOINT
%% =========================
subgraph TRIAGE[Triage, Review, and Reopen]
  T_QUEUE[Deterministic queue ranking
  ...
  recovery, replay, reconciliation,
...
%% ENDPOINTS
%% =========================
subgraph ENDPOINTS[Resolution and Handoff Paths]

  E_SELFCARE[Self-care advice
  safety net and instructions]
  E_ADMIN[Admin resolution case]
  ...
  E_CALLBACK[CallbackCase]
  E_CALLBACK_TRY[Attempt windows, voicemail policy,
  retry limits, escalation]
  E_CALLBACK_OUTCOME[Attempt outcome, repair,
  retry, or escalation recorded]

  E_BOOK_GATE[Provider capability matrix
  patient self-service vs staff assist]
  ...
  eligibility policy]
  E_PHARM_FIND[Directory snapshot
  opening hours and patient choice]
  ...
  E_PHARM_BOUNCE[Routine bounce-back reopen]
  E_PHARM_URGENT[Urgent pharmacy return]

end

T_OUTCOME -->|Self-care| E_SELFCARE
T_OUTCOME -->|Admin| E_ADMIN
T_HANDOFF -->|Clinician message| E_MSG
T_HANDOFF -->|Callback| E_CALLBACK
T_HANDOFF -->|Local booking| E_BOOK_GATE
T_HANDOFF -->|Hub booking| E_HUB_CASE
T_HANDOFF -->|Pharmacy First| E_PHARM_ELIG

E_MSG --> E_MSG_DELIV
E_MSG_DELIV -->|Reply, closure, or routine thread work| E_MSG_OUTCOME
E_MSG_DELIV -->|Delivery failure or invalid route| X_CONTACT
E_MSG_OUTCOME -->|New clinical evidence or contact-risk delta| T_RESPONSE

E_CALLBACK --> E_CALLBACK_TRY
E_CALLBACK_TRY -->|Availability update, repair, or attempt result| E_CALLBACK_OUTCOME
E_CALLBACK_TRY -->|Route invalid or dependency failure| X_CONTACT
E_CALLBACK_OUTCOME -->|Retry or reschedule| E_CALLBACK
E_CALLBACK_OUTCOME -->|New clinical evidence or reachability risk| T_RESPONSE
X_CONTACT -->|Repaired with material delta| T_RESPONSE
X_CONTACT -->|Repaired with non-clinical delta| PV_COMMS
X_CONTACT -->|Needs recovery action| PV_RECOVER

E_BOOK_GATE -->|Unsupported or degraded| E_HUB_CASE
...
E_HUB_CASE --> E_HUB_RANK
E_HUB_RANK -->|Direct coordinator commit| E_HUB_PENDING
E_HUB_RANK -->|Alternative offer| E_HUB_ALT
E_HUB_ALT -->|Accepted option| E_HUB_PENDING
E_HUB_ALT -->|Callback requested| E_CALLBACK
E_HUB_ALT -->|Declined or expired| E_HUB_CASE
E_HUB_ALT -->|New clinical or reachability risk| T_RESPONSE
E_HUB_PENDING -->|Confirmed| E_APPT
E_HUB_PENDING -->|Urgent or clinically material return| T_RESPONSE
E_HUB_PENDING -->|Disputed, stale, or degraded no-ack path| T_EXCEPTION

%% Architectural correction: pharmacy choice is insufficient on its own.
%% A live referral exists only after explicit consent plus durable dispatch proof.
...
subgraph PATIENT_VIEW[Patient Account and Communications]
  ...
  PV_RECOVER[Recovery and identity hold
  expired actions, consent renewal,
  wrong-patient and secure re-entry]
end
...
%% =========================
subgraph STAFF_USERS[Staff and Operations Users]
  SU_CLIN[Clinician or practice staff
  clinical workspace]
...
%% =========================
subgraph ADMIN_USERS[Admin and Governance Users]
  AU_CONFIG[Config admin
  compiled policy bundles and approvals]
...
subgraph PLATFORM[Platform Data, Policy, and Assurance Spine]
  ...
  effective-at, canary, rollback]
  PL_POLICY[Compiled policy and config registry
  active and future-effective bundles]
  PL_BUS[Outbox and event bus
  causal tokens and correlation IDs]
  PL_READ[Scope-aware read projections
  ...
  degraded promises, or PHI-bearing grants remain]
end
...
I_FAIL --> PL_WRITE
I_REPAIR --> PL_WRITE
T_EXCEPTION --> PL_WRITE
T_DECISION --> PL_WRITE
T_OUTCOME --> PL_WRITE
E_MSG_OUTCOME --> PL_WRITE
E_CALLBACK_OUTCOME --> PL_WRITE
E_APPT --> PL_WRITE
E_PHARM_CONSENT --> PL_WRITE
E_PHARM_DONE --> PL_WRITE
PL_POLICY --> I_AUTH
PL_POLICY --> I_SAFETY
PL_POLICY --> I_REPAIR
PL_POLICY --> T_QUEUE
PL_POLICY --> E_MSG
PL_POLICY --> E_BOOK_GATE
...
PL_READ --> T_QUEUE
PL_READ --> E_HUB_RANK
PL_READ --> PL_ANALYTICS --> SU_OPS
PL_AUDIT --> PL_ASSURE --> AU_GOV

AU_CONFIG --> PL_PROMOTE
AU_COMMS --> PL_PROMOTE
AU_ACCESS --> PL_PROMOTE
PL_PROMOTE --> PL_POLICY
PL_QUAR --> SU_OPS

%% =========================
%% INTEGRATION RAILS
%% =========================
subgraph RAILS[External Integration Rails]
  R_ID[NHS login and identity rail]
  R_GP[GP system adapters
  IM1-first and local gateways]
  R_MSG[Messaging adapters
  SMS, email, push]
  R_TEL[Telephony adapters]
  R_DIR[Directory and service discovery]
  R_REF[Referral transport
  partner inbox or secure rail]
end

R_ID --> I_AUTH
R_TEL --> I_CAPTURE
E_CALLBACK_TRY --> R_TEL
E_BOOK_SEARCH --> R_GP
E_BOOK_CONFIRM --> R_GP
E_BOOK_PENDING --> R_GP
E_HUB_PENDING --> R_GP
...
R_TEL --> PL_BUS
R_REF --> PL_BUS
PL_BUS -.->|Exception, timeout, or bad ack| T_EXCEPTION
PL_BUS -.->|Binding anomaly or wrong-patient signal| I_REPAIR

%% =========================
%% CLOSURE
%% =========================
CLOSE[Request closes only after outcome is recorded,
audit is durable, command-following reads have settled,
no open lease, duplicate review, preemption,
reconciliation, identity hold, degraded exception,
or active PHI-bearing grant remains]

E_SELFCARE --> PL_CLOSE
E_ADMIN --> PL_CLOSE
E_MSG_OUTCOME --> PL_CLOSE
E_CALLBACK_OUTCOME --> PL_CLOSE
E_APPT --> PL_CLOSE
E_PHARM_DONE --> PL_CLOSE
PL_CLOSE --> CLOSE
